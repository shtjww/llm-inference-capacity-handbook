...
**【定量：混合负载的算力错配】**Kimi 生产 trace 实测：>32K 的请求只占 3.2%，却吃掉 21% 的 prefill 算力；>P95 的 5% 请求吃掉 27%。两个规划推论：① 容量公式里的 L_in 要用**算力加权均值**（ΣL²/ΣL）而不是请求均值——用请求均值等于假装这部分开销不存在，λ_compute 系统性高估；② 混合负载的 Cs²（实测 ≈1.4）是齐整负载（≈0.35）的 4 倍——排队税按混合口径取，别按教科书取 1。
